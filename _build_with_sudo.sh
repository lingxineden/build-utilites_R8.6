...
{
	sudo  mkdir -p `path filesystem`/usr/bin
	sudo  mkdir -p `path filesystem`/etc
	sudo  mkdir -p `path filesystem`/etc/init.d
	sudo  mkdir -p `path filesystem`/etc/rcS.d
	sudo  mkdir -p `path filesystem`/usr/lib/crda
	sudo  mkdir -p `path filesystem`/lib/firmware/ti-connectivity
	sudo  mkdir -p `path filesystem`/usr/share/wl18xx
	sudo  mkdir -p `path filesystem`/usr/sbin/wlconf
	sudo  mkdir -p `path filesystem`/usr/sbin/wlconf/official_inis
        sudo  mkdir -p `path filesystem`/etc/wireless-regdb/pubkeys
}
...
function setup_toolchain()
{
	if [ ! -f `path downloads`/arm-toolchain.tar.bz2 ]; then
		...
		sudo  tar -xjf `path downloads`/arm-toolchain.tar.bz2 -C `path toolchain`
...
{
    cd_repo kernel
	[ -z $NO_CONFIG ] && sudo  cp `path configuration`/kernel_$KERNEL_VERSION.$KERNEL_PATCHLEVEL.config `repo_path kernel`/.config
    ...
    then
        ...
        sudo  cp `repo_path kernel`/arch/arm/boot/uImage `path tftp`/uImage
    else
        if [ -z $NO_DTB ] 
        then
...
sudo PATH=$PATH -E  	    INSTALL_MOD_PATH=`path filesystem` make -j${PROCESSORS_NUMBER} modules_install
            sudo  cp `repo_path kernel`/arch/arm/boot/zImage `path tftp`/zImage
            sudo  cp `repo_path kernel`/arch/arm/boot/dts/am335x-*.dtb `path tftp`/
        else
            LOADADDR=0x80008000 make -j${PROCESSORS_NUMBER} uImage.am335x-evm 
            sudo  cp `repo_path kernel`/arch/arm/boot/uImage.am335x-evm `path tftp`/uImage
        fi
    fi
	assert_no_error
	cd_back
...
function build_modules()
{
	...
	assert_no_error
	#find . -name \*.ko -exec sudo  cp {} `path debugging`/ \;
	find . -name \*.ko -exec ${CROSS_COMPILE}strip -g {} \;
    
sudo PATH=$PATH -E  	make  modules_install
	assert_no_error
	cd_back
}
...
function build_openssl()
{
	...
	make
	assert_no_error
sudo PATH=$PATH -E  	make install_sw
	assert_no_error
	cd_back
}
...
function build_iw()
{
	...
	assert_no_error
sudo PATH=$PATH -E  	DESTDIR=`path filesystem` make install
	assert_no_error
	cd_back
}
...
function build_wpa_supplicant()
{
	...
	[ -z $NO_CONFIG ] && sudo  cp android.config .config
	...
	assert_no_error
sudo PATH=$PATH -E  	CONFIG_LIBNL32=y DESTDIR=`path filesystem` make install
	assert_no_error
	cd_back    
    sudo  cp `repo_path scripts_download`/conf/*_supplicant.conf  `path filesystem`/etc/
}
...
sudo PATH=$PATH -E  	CONFIG_LIBNL32=y DESTDIR=`path filesystem` make install
	assert_no_error
	cd_back
    sudo  cp `repo_path scripts_download`/conf/hostapd.conf  `path filesystem`/etc/    
}
...
function build_calibrator()
{
	...
	assert_no_error
sudo PATH=$PATH -E  	NFSROOT=`path filesystem` make install
	#assert_no_error
	sudo  cp -f `repo_path ti_utils`/hw/firmware/wl1271-nvs.bin `path filesystem`/lib/firmware/ti-connectivity
	cd_back
}
...
function build_wlconf()
{
	...
	if [ -z $NO_CLEAN ]; then
		...
		for file_to_copy in $files_to_copy; do
			sudo  rm -f `path filesystem`/usr/sbin/wlconf/$file_to_copy
		done
		sudo  rm -f `path filesystem`/usr/sbin/wlconf/official_inis/*
	fi
	NFSROOT=`path filesystem` make CC=${CROSS_COMPILE}gcc LD=${CROSS_COMPILE}ld
	assert_no_error

	# install
	sudo  cp -f `repo_path ti_utils`/wlconf/wlconf `path filesystem`/usr/sbin/wlconf
	sudo  chmod 755 `path filesystem`/usr/sbin/wlconf
	for file_to_copy in $files_to_copy; do
		sudo  cp $file_to_copy `path filesystem`/usr/sbin/wlconf/$file_to_copy
		echo "echoying files $file_to_copy"
	done
	sudo  cp official_inis/* `path filesystem`/usr/sbin/wlconf/official_inis/
	cd_back
}

function build_fw_download()
{
	sudo  cp `repo_path fw_download`/*.bin `path filesystem`/lib/firmware/ti-connectivity
}
...
function build_uim()
{
	...
	assert_no_error
        sudo  install -m 0755 uim `path filesystem`/usr/bin
	sudo  install -m 0755 `repo_path uim`/scripts/uim-sysfs `path filesystem`/etc/init.d/
	cd `path filesystem`/etc/rcS.d/
	sudo  ln -sf  ../init.d/uim-sysfs S03uim-sysfs
	assert_no_error
	cd_back
}
...
function build_bt_firmware()
{
	...
	for i in `repo_path bt-firmware`/initscripts/*.bts; do
		...
		sudo  install -m 0755 $i `path filesystem`/lib/firmware/
		assert_no_error
	done
}
...
function build_scripts_download()
{
	...
	do
		...
		sudo  cp -rf ${script_dir}/* `path filesystem`/usr/share/wl18xx
	done
	cd_back
}
...
function clean_outputs()
{
    ...
    then
        ...
        sudo  rm -rf `path filesystem`/*
        sudo  rm -f `path outputs`/*
   fi
}
...
function build_outputs()
{
    ...
    then  
        ...
        cd_path filesystem
        sudo  tar cpjf `path outputs`/${tar_filesystem[0]} .
        cd_back
		...
		then
			if [ "$KERNEL_VERSION" -eq 3 ] && [ "$KERNEL_PATCHLEVEL" -eq 2 ]
			then
				sudo  cp `path tftp`/uImage `path outputs`/uImage
			else
				if [ -z $NO_DTB ]
				then
					sudo  cp `path tftp`/zImage `path outputs`/zImage
					sudo  cp `path tftp`/*.dtb `path outputs`/
				else
					sudo  cp `path tftp`/uImage `path outputs`/uImage
				fi
			fi
...
function install_outputs()
{
	...
	sudo  cp `path outputs`/uImage ${tftp_path}
	sudo  cp `path outputs`/${tar_filesystem[0]} $sitara_left_path
	sudo  cp `path outputs`/${tar_filesystem[0]} $sitara_right_path

	cd $sitara_left_path
	sudo  tar xjf ${tar_filesystem[0]}
	cd_back
